...
var chunkGUO4USPX_cjs = require('./chunk-GUO4USPX.cjs');
var chunkXEG6MMZC_cjs = require('./chunk-XEG6MMZC.cjs');
...
var Bundler = class extends bundler.MastraBundler {
  ...
  async writeInstrumentationFile(outputDirectory, customInstrumentationFile) {
    ...
    const __dirname = path.dirname(url.fileURLToPath((typeof document === 'undefined' ? require('u' + 'rl').pathToFileURL(__filename).href : (_documentCurrentScript && _documentCurrentScript.tagName.toUpperCase() === 'SCRIPT' && _documentCurrentScript.src || new URL('chunk-FL6AAU4I.cjs', document.baseURI).href))));
  ...
  createBundler(inputOptions, outputOptions) {
    return chunkXEG6MMZC_cjs.createBundler(inputOptions, outputOptions);
  }
  async analyze(entry, mastraFile, outputDirectory, { enableEsmShim = true } = {}) {
    return await chunkDFG5D3TX_cjs.analyzeBundle(
      [].concat(entry),
      mastraFile,
      {
  ...
  async getBundlerOptions(serverFile, mastraEntryFile, analyzedBundleInfo, toolsPaths, { enableSourcemap = false, enableEsmShim = true } = {}) {
    const { workspaceRoot } = await chunkDFG5D3TX_cjs.getWorkspaceInformation({ mastraEntryFile });
    ...
    const inputOptions = await chunkXEG6MMZC_cjs.getInputOptions(
      mastraEntryFile,
      analyzedBundleInfo,
      "node",
      {
  ...
  async _bundle(serverFile, mastraEntryFile, {
    ...
    let sourcemap = false;
    try {
      const bundlerOptions = await chunkDFG5D3TX_cjs.getBundlerOptions(mastraEntryFile, analyzeDir);
    ...
    try {
      ...
      analyzedBundleInfo = await chunkDFG5D3TX_cjs.analyzeBundle(
    ...
    try {
      const result = await chunkGUO4USPX_cjs.writeTelemetryConfig(mastraEntryFile, path.join(outputDirectory, this.outputDir));
    ...
    for (const dep of analyzedBundleInfo.externalDependencies) {
      try {
        ...
        const rootPath = await chunkDFG5D3TX_cjs.getPackageRootPath(dep);
...
//# sourceMappingURL=chunk-FL6AAU4I.cjs.map
//# sourceMappingURL=chunk-FL6AAU4I.cjs.map
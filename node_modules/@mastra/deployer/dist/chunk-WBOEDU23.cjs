    ...
    resolveId(id, importer) {
      ...
      try {
        ...
        for (const ext of [".mjs", ".js", ".cjs"]) {
          ...
          if (resolved2) {
            const pkgName = chunkDFG5D3TX_cjs.getPackageName(id);
            if (!pkgName) {
              return null;
            }
...
async function getInputOptions2(entryFile, platform, env, { sourcemap = false } = {}) {
  ...
  const projectRoot = closestPkgJson ? path.dirname(chunkDFG5D3TX_cjs.slash(closestPkgJson)) : chunkDFG5D3TX_cjs.slash(process.cwd());
  const { workspaceMap, workspaceRoot } = await chunkDFG5D3TX_cjs.getWorkspaceInformation({ mastraEntryFile: entryFile });
  const analyzeEntryResult = await chunkDFG5D3TX_cjs.analyzeBundle(
    [entryFile],
    entryFile,
    {
  ...
  for (const [dep, metadata] of analyzeEntryResult.dependencies.entries()) {
    const pkgName = chunkDFG5D3TX_cjs.getPackageName(dep);
  ...
  if (Array.isArray(inputOptions.plugins)) {
    ...
    inputOptions.plugins.push(chunkDFG5D3TX_cjs.aliasHono());
...
function removeAllOptionsExceptServer(result, logger) {
  return chunkDFG5D3TX_cjs.removeAllOptionsFromMastraExcept(result, "server", logger);
}

// src/build/serverOptions.ts
async function getServerOptions(entryFile, outputDir, logger) {
  const result = await chunkDFG5D3TX_cjs.extractMastraOption(
    "server",
    entryFile,
...
//# sourceMappingURL=chunk-WBOEDU23.cjs.map
//# sourceMappingURL=chunk-WBOEDU23.cjs.map
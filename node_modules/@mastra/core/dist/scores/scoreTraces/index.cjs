...
function normalizeMessageContent(content) {
  ...
  const converted = chunkAPY2BE4K_cjs.convertMessages(tempMessage).to("AIV4.UI");
...
function convertToUIMessage(message, createdAt) {
  ...
  const converted = chunkAPY2BE4K_cjs.convertMessages(messageInput).to("AIV4.UI");
...
var getTraceStep = chunkXHPBKALR_cjs.createStep({
...
async function validateAndSaveScore({ storage, scorerResult }) {
  const payloadToSave = chunkKAV4EP3T_cjs.saveScorePayloadSchema.parse(scorerResult);
...
var scoreTracesWorkflow = chunkXHPBKALR_cjs.createWorkflow({

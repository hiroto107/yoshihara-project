...
var MastraScorer = class _MastraScorer {
  ...
  toMastraWorkflow() {
    const workflowSteps = this.steps.map((scorerStep) => {
      return chunk462DRETM_cjs.createStep({
        id: scorerStep.name,
    ...
    const workflow = chunk462DRETM_cjs.createWorkflow({
  ...
  async executePromptStep(scorerStep, tracingContext, context) {
    ...
    const resolvedModel = await chunkDGQW5VRR_cjs.resolveModelConfig(modelConfig);
    const judge = new chunk462DRETM_cjs.Agent({
      name: "judge",
      model: resolvedModel,
      instructions,
    ...
    if (scorerStep.name === "generateScore") {
      ...
      if (resolvedModel.specificationVersion === "v2") {
        result = await chunk462DRETM_cjs.tryGenerateWithJsonFallback(judge, prompt, {
          structuredOutput: {
            schema
          },
          tracingContext
...
function isWorkflow(target) {
  return target instanceof chunk462DRETM_cjs.Workflow;
}
...
//# sourceMappingURL=chunk-AJK7QPSD.cjs.map
//# sourceMappingURL=chunk-AJK7QPSD.cjs.map